...
fun solve(input: List<String>) {
    ...
    var minAreaSize = Long.MAX_VALUE
    var minAreaTime = 0
    for (t in 0..100000) {
        val area = computeArea(points)
        val topLeft = area[0]
        val bottomRight = area[1]

        val width = bottomRight.x - topLeft.x
        val height = bottomRight.y - topLeft.y
        val size: Long = width.toLong() * height.toLong()
...
fun printImage(input: List<String>, minAreaTime: Int, numOfPrints: Int) {
    val points = parsePoints(input)
    val t = minAreaTime - (numOfPrints - 1) / 2
    move(points, t)

    for (i in 1..numOfPrints) {
        val area = computeArea(points)
        println("After ${t + i - 1} seconds:")
        println(computeImage(points, area[0], area[1]).joinToString("\n"))
        move(points, 1)
    }
}
